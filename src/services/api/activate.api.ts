...
const ActivateApi: Activate.ActivateAPI = {
	...
	checkSim: async (param) => {
		return axiosInstance.post('/bss/videocall/check-sim-exist', param).then(res => res.data);
	},
	getKycFront: async (image_front_base64) => {
		return axiosInstance.post('/ekyc/v1/front', { image_front_base64 }, {
			headers: configVideoCall,
			baseURL: baseURL
		}).then(res => res.data);
	},
	checkIsCanRegisterSim: async (id) => {
		return axiosInstance.post(`/bss/videocall/check-number-sim-registered`, { id_number: id }).then(res => res.data);
	},
	getKycOcr: async (image_front_base64, image_back_base64) => {
		return axiosInstance.post('/ekyc/v1/ocr', { image_front_base64, image_back_base64 }, {
			headers: configVideoCall,
			baseURL: baseURL
		}).then(res => res.data);
	},
	getFaceMatch: async (image_front_base64, image_face_base64) => {
		return axiosInstance.post('/ekyc/v1/face_matching', { image_front_base64, image_face_base64 }, {
			headers: configVideoCall,
			baseURL: baseURL
		}).then(res => res.data);
	}
